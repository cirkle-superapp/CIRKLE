 ...
 * - Rotates a wrapper `<div>` (HTML transform-origin is 100% reliable
 *   across browsers, unlike SVG transform-box).
 * - The gradient uses `gradientUnits="userSpaceOnUse"` spanning the
 *   whole 0 0 100 100 viewBox, so the gold→rose→teal sweep is GLOBAL
 *   to the mark — making the rotation visibly obvious (otherwise each
 *   circle would have its own local gradient and the 3-fold symmetry
 *   would hide the rotation).
 * - References a single shared gradient `cirkle-brand-grad` rendered
 *   once by <CirkleBrandDefs /> (no per-instance useId → no SSR
 *   hydration mismatch).
 */
    ...
    <div className={cn("flex items-center gap-2.5", className)}>
      ...
      {withWordmark && (
        <div className={cn("flex flex-col leading-none", wordmarkClassName)}>
          ...
          <span className="text-[0.65em] text-muted-foreground -mt-0.5">your connected world</span>
        </div>
      )}
    </div>
  );
